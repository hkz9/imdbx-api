...
export default async function getTitle(id) {
  ...
  return {
    ...
    contentType: props.aboveTheFoldData?.titleType?.id ?? "Unknown",
    contentRating: props.aboveTheFoldData?.certificate?.rating ?? "N/A",
    isSeries: props.aboveTheFoldData?.titleType?.isSeries ?? false,
    productionStatus:
      props.aboveTheFoldData?.productionStatus?.currentProductionStage?.id ?? "Unknown",
    isReleased:
    ...
    images: props.mainColumnData?.titleMainImages?.edges
      ...
      ?.map((e) => e.node.url) ?? [],
    plot: props.aboveTheFoldData?.plot?.plotText?.plainText ?? "No plot available",
    runtime:
    ...
    releaseDetailed: {
      date: props.aboveTheFoldData?.releaseDate ? new Date(
